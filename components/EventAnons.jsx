  ...
  const events = useMemo(() => [
    {
      id: 1,
      date: "14-16 февраля",
      title: "3D: Доверия , Достоинство , Действие.",
      desc: "Милые! Любимые! Ласковые! Как порой не хватает нежности и поддержки! И мы обижаемся, гневаемся, часто виним себя за что то...",
      about: "В долгих рассуждениях у нас родилось выездное мероприятие, наполненное любовью и радостью. Приглашаем вас в чудесное путешествие! В котором обсудим и применим алгоритм  ВЫХОДА из Боли и Страха , из СоЗависимости и Терпения !Ты получишь алгоритм вывода из тупика. Увидишь множество дорог ведущих в новые состояния Себя. Осознаешь причины потери жизненной энерги и научишься закрывать эти пробоины.",
      plan: ["Практики Трансформации", "Диагностика", "Очищение", "Наполнение", "Медитации с ПОЮЩИМИ Тибетскими ЧАШАМИ",]
    },
    {
      id: 2,
      date: "18-19 февраля",
      ...
      <Carousel setApi={setApi}>
        <CarouselContent>
          {events.map(event => (
            <CarouselItem
              key={event.id}
              className={`md:basis-1/3 basis-1/2 mx-2 p-4 border rounded cursor-pointer ${selectedEvent?.id === event.id ? 'border-[var(--accent-color)]' : 'border-gray-300'}`}
